...
{
    return WaitForSingleObject(service.event, 0) != WAIT_OBJECT_0 ? 1 : 0;
}
...
int main(int argc, TCHAR *argv[])
{
    // parse the command
    {
        ...
        service.path = std::string(command, command + pos + 1);
        service.exec = std::string(command + pos + 1);
    }
